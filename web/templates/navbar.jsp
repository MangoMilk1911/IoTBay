    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <div class="navbar-nav w-100">
                ...
                <% if (user != null) { %>
                <a class="nav-item nav-link" href="${pageContext.request.contextPath}/EditProfileServlet">Edit
                    Profile</a>
                ...
                <c:if test="${user.staff}">
                    <a class="nav-item nav-link"
                       href="${pageContext.request.contextPath}/ReportingServlet">Reporting</a>
                </c:if>
                <c:if test="${user.staff && user.admin}">
                    <a class="nav-item nav-link" href="${pageContext.request.contextPath}/UserManagementServlet">User
                        Management</a>
                </c:if>
                <% } else { %>
                ...
                <c:if test="${user.customer}">
                    <a class="btn btn-outline-light my-2 my-sm-0"
                       href="${pageContext.request.contextPath}/checkout.jsp">Shopping Cart</a>
                </c:if>
                <form class="form-inline mb-0 px-4" action="${pageContext.request.contextPath}/LogoutServlet"
                      method="post">
